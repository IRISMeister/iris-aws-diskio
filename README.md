...
GUIを伴わない[コマンドライン](perf.bat)での実行が可能。結果は[CSV出力](counters_2022104135.csv)できる。
...
![phywrs-and-writebytes](phywrs-and-writebytes.png)

> これに相当するCloudWatchの指標はVolumeWriteBytes/合計。ただし、CloudWatchの最短の計測単位は1分(mgstat,perfmonは1秒)なので、同じ様な「山と谷の」グラフにはならないので注意。  
> また、IRISのデータベースへの書き込みサイズは可変長なので、PhyWrsはIOPS(CloudWatchのVolumeWriteOps)とは必ずしも連動しない。
